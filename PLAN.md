...
## 2026-08-15 · 누설 경로는 팔 입력이 아니라 몸통 입력이었다

부위별 전문가로 "팔 입력을 다리에 안 주면 누설 0"이라 결론냈는데, 사용자는 라이브에서
다리가 계속 움직인다고 지적했다. **그 0.000000°는 팔 축만 흔든 합성 검사에서 나온
숫자였고, 실제 인지 출력은 팔↔다리 입력 상관 +0.718, 팔↔몸통 +0.815다.** 팔을 들면
몸통 입력이 움직이고, 다리 전문가는 몸통 입력을 받고 있었다.

`fixtures/soma_part77.npy`의 "torso" 13관절에 **쇄골(11, 39)과 머리 체인(4-10)이
포함**돼 있는 것이 구체적 경로다. 쇄골은 팔을 들 때마다 회전한다.

측정 도구: `data/legstatic_test_k1.npz` — 다리 관절을 완전히 고정하고 상체만 움직이는
합성 4스탠스 1,200프레임. 여기서 나오는 다리 움직임은 정의상 전부 누설이다.

| | 홀드아웃 | 다리 std | 다리 프레임간 |
|---|---|---|---|
| 교사 | — | 2.546° | 0.1555° |
| `parts` (torso 전부) | 9.14° | 4.110° | 0.2836° |
| **`--part_in tight`** | **9.01°** | 3.946° | **0.2394°** |
| `--fuse` | 9.11° | 4.138° | 0.2867° |
| `tight --fuse` | 8.99° | 3.951° | 0.2406° |
| `tight` + 다리고정 데이터 | 9.60° | 3.474° | 0.2217° |

**입력을 줄였는데 정확도가 올랐다** (다리 전문가 66→39차원, 5.93→5.69°). 융합 보정층은
기각 — 조립된 23각도에는 전문가가 놓친 정보가 없다. `--fuse`로 남긴다.

기본 모델을 `models/k1_retarget_tight.pt`로 옮겼다(세 지표 전부 우세).
영상 `outputs/tight_split.mp4`.

## 2026-08-15 · 튐은 학생이 교사 불연속을 2.2배 증폭한 것

정확도 우선 라이브(imgfeat 켬, 16~20 FPS) 중 "몇몇 각도에서 튄다"는 지적. 상위 1% 점프
프레임에서 인지 입력은 2.89° 움직이는데(평소 0.59°) 교사가 55.6°, **학생이 122.3°**
점프한다. 관절은 전부 어깨 pitch/yaw와 팔꿈치 — twist DOF 문제와 같은 자리다.

`motion_command.py`의 OneEuro를 리타겟 출력에 재사용(`--smooth`, 기본 2.0 Hz).
15 fps 기준 홀드아웃 9.01 → **8.93°**(개선), 최악 점프 237.9 → **108.1°**.
학생의 프레임간 노이즈가 교사보다 크므로 저역통과가 정확도를 깎지 않는다.

- `beta`는 0 고정. 속도 적응은 튀는 순간에 cutoff를 넓혀 튐을 통과시킨다.
- 필터에 **측정 dt**를 넘긴다. 3 Hz의 tau가 53 ms라 16 fps(62 ms)에서는 거의 안 걸리므로,
  고정 dt를 가정하면 프레임레이트가 흔들릴 때 스무딩 양이 조용히 바뀐다.
  `motion_command.py` 자체검사가 이 성질을 검증한다.
- 근본 원인(교사 자신의 55.6° 불연속)은 남아 있다. 학습 연속성 손실 또는 twist DOF를
  각도가 아닌 표현으로 예측하는 쪽이 진짜 해결.

## 2026-08-15 · 오염 경로는 쇄골이 아니라 척추 (`--part_in solo`)

`tight`로도 "팔의 특정 각도에서 하체가 오염된다"는 지적이 계속돼, 입력을 부위별로
섭동해 구조적 경로를 직접 쟀다 (±20° → 다리 출력 변화, 평균/최대):

| 섭동 | parts | tight | solo |
|---|---|---|---|
| 팔 축 | 0.000 | 0.000 | 0.000 |
| 쇄골 | 5.96 / 168.0 | 0.000 | 0.000 |
| **척추** | 17.67 / 187.8 | **19.05 / 217.3** | **0.000** |
| 골반 위 전부 | 18.00 / 188.8 | 19.54 / 244.5 | **0.000** |

쇄골을 끊어도 **척추 경로가 더 크게 남아 있었다.** 인지 출력에서 척추는 팔과 상관 +0.737로
같이 움직이고, 팔이 빠른 상위 5% 프레임에서는 평소의 4.3배로 움직인다.

`--part_in solo`(다리 ← 다리 30차원만): 홀드아웃 9.01 → **9.12°**(+0.11), 다리고정
테스트 다리 std 3.946 → **0.000°**. 기본 모델을 여기로 옮겼다.
교사의 몸통→다리 커플링 2.546°를 포기하는 대신, 상체가 다리를 못 건드린다.

교훈: "팔 입력을 안 준다"는 누설을 막지 못한다. 인지가 상체 움직임을 몸통 회전으로
쏟아내기 때문에, 막아야 하는 것은 **골반 위 전부**다. 부위별 섭동으로 경로를 재라 —
합성 팔 섭동만으로는 세 번 연속 0을 보고 세 번 다 틀렸다.
