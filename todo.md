...
- [x] **특정 각도에서 튐** — 학생이 교사 불연속(55.6°)을 122.3°로 증폭. 어깨 pitch/yaw,
  팔꿈치. `--smooth 2.0`(OneEuro 재사용)으로 최악 점프 238→108°, 정확도는 9.01→8.93°로
  오히려 개선.
- [ ] **튐의 근본 원인** — 교사 자신의 55.6° 불연속. 필터는 가릴 뿐이다. 학습에 연속성
  손실을 넣거나, twist DOF를 각도가 아닌 표현(위치로 결정되는 것)으로 예측할 것
- [x] **하체 오염 — 척추 경로** — 쇄골을 끊어도(`tight`) 척추 → 다리 경로가 최대 217°로
  남아 있었고, 인지 척추는 팔과 상관 +0.737로 함께 움직인다. `--part_in solo`(다리는
  다리 입력만)로 골반 위 섭동에 대한 다리 변화가 **0.000°**. 비용 +0.11°(9.01→9.12°).
  기본 모델을 `models/k1_retarget_solo.pt`로 옮겼다
